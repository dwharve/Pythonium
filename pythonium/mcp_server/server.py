...
This module provides an MCP server implementation that uses the official
MCP SDK's FastMCP class instead of our custom implementation.
"""
...
class MCPSDKServerError(PythoniumError):
...
class PythoniumMCPServer:
    ...
    def __init__(
        ...
        logger.info(
            f"Pythonium MCP Server initialized with transport: {self.config.transport.type}"
        )
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            await self._cleanup()
            raise MCPSDKServerError(f"Server startup failed: {e}")
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            transport_type = self.config.transport.type.lower()
